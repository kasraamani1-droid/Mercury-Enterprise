...
2. **Maintenance** — Planning, Work Orders, MRO Execution, Logbook, Engineering, Technical Library, Personnel  
...
- Cross-module: Aircraft → Work Order → Job Card → Materials (MR) → Logistics part/PO; Planning due/check/AD/finding → generate WP → Work Order → Aircraft → Configuration; Aircraft/Component Publications and AD/SB/EO `publication_id` → publication object → Technical Library; Job card assigned employees → Personnel. Area tabs stay open; `data-we-tab` selects an object tab without resetting unrelated sessions.

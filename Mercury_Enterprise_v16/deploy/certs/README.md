...
4. `sh deploy/init-letsencrypt.sh` (uses the production overlay so `:3000` stays unpublished).
5. `docker compose --profile production -f docker-compose.yml -f docker-compose.production.yml up --build -d`

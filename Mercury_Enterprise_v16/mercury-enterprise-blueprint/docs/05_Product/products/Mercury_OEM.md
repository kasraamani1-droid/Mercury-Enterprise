...
Mercury OEM connects manufacturers (aircraft, engine, component) to in-service configuration feedback, applicability, and service-bulletin effectiveness without replacing PLM systems of record.

---

## 2. Positioning

Mercury OEM is a named commercial and architectural product within the Mercury Aviation Enterprise Operating System. It does not ship a separate database or a divergent security model. It packages capabilities on **Mercury Core** so buyers recognize their operating role while the **Digital Thread** remains continuous.

---

## 3. Target users

- Airworthiness / service engineering
- Fleet support
- Spares and publications distribution partners
...
| Capability | Status |
|------------|--------|
| Component catalog / ATA / alternates | Delivered foundation |
| Publications and applicability | Partial |
| OEM portal and fleet telemetry packs | Planned |
| IPC commercial distribution | Planned |

---

## 5. Major entities

ComponentCatalogItem, AlternatePart, Publication, Applicability rules (target), ServiceBulletin effectiveness metrics (target).
...
`/api/v1/components/*`, `/publications/*`; future `/oem/*` packs.
...
Core + Passport for in-service truth; Connect for PLM; Marketplace for spares listings later.

---

## 11. Roadmap

Cross-org sharing constructs; SB effectiveness dashboards; authorized shop network.

...
Mercury Core is the AEOS substrate: multi-tenant organizations, sessions, RBAC, audit engine, API gateway surface, and shared reference data required by every other Mercury product.

---

## 2. Positioning

Mercury Core is a named commercial and architectural product within the Mercury Aviation Enterprise Operating System. It does not ship a separate database or a divergent security model. It packages capabilities on **Mercury Core** so buyers recognize their operating role while the **Digital Thread** remains continuous.
...
- All operational personas (indirectly)
...
| Capability | Status |
|------------|--------|
| Organizations, sites, memberships | Delivered |
| Session auth | Delivered |
| Central RBAC | Delivered |
| Audit engine | Delivered |
| OpenAPI /api/v1 | Delivered |
| Shared ATA / manufacturers (via modules) | Delivered |

---

## 5. Major entities

Organization, Site, Membership, Operator/Session, AuditEvent, Role/Permission grants.
...
`/api/v1/org/*`, auth/session endpoints, audit list APIs, platform health.
...
None — Core is the root. Consumed by all products.

---

## 11. Roadmap

OIDC/SSO/MFA; Redis session store; finer persona overlay engine; directory sync.
